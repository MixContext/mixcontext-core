...
// cli/index.ts
var chalk = import_chalk.default;
async function main() {
...
`).parse();
  ...
  if (opts.noColor) {
    chalk = new import_chalk.Chalk({ level: 0 });
  }
  if (opts.init) {
    const configPath = import_path.default.resolve(process.cwd(), "mixcontext.config.ts");
    let exists = false;
    try {
...
`;
      await import_fs.promises.writeFile(configPath, template, "utf8");
      console.log(chalk.green(`Created ${import_path.default.relative(process.cwd(), configPath)}`));
    } else {
      console.log(chalk.yellow("mixcontext.config.ts already exists \u2013 nothing to do."));
    }
    process.exit(0);
  }
  const matched = await (0, import_fast_glob.default)(patterns, { absolute: true, dot: false });
  const uniqueFiles = Array.from(new Set(matched));
  if (uniqueFiles.length === 0) {
    console.error(chalk.red("No input files matched the given patterns."));
    process.exit(2);
  }
  const multibar = new import_cli_progress.default.MultiBar(
    {
  ...
  for (const file of uniqueFiles) {
    try {
      const buf = await import_fs.promises.readFile(file);
      const buildOpts = {
      ...
      doc.filename = import_path.default.basename(file);
      if (!doc.chunks) {
      ...
      if (opts.vectorDb === "local" && opts.embed && doc.chunks) {
        for (const chunk of doc.chunks) {
          ...
          const chunkId = `${doc.id ?? import_path.default.basename(file)}-${chunk.idx}`;
      ...
      if (opts.out && uniqueFiles.length === 1) {
        dest = import_path.default.resolve(String(opts.out));
      } else {
        dest = import_path.default.join(
          import_path.default.dirname(file),
          `${import_path.default.basename(file, import_path.default.extname(file))}.mcx.zip`
        );
      }
      await import_fs.promises.writeFile(dest, zipBuf);
  ...
  console.table(
    summary.map((s) => ({
      File: import_path.default.basename(s.file),
      Tokens: s.tokens,
      Chunks: s.chunks,
